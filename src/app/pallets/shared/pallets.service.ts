...
export class PalletsService {
  ...
  getInTransitOff(branch: string, pallet: string): Observable<Pallet[]> {
    ...
    let url = this.palletTrackerUrl + `/items?expand=fields(select=${pallet})`;
    ...
    return this.http.get(url).pipe(map((_: any) => _.value.reduce((acc, val) => acc + (val['fields'][pallet] || val['fields'][pallet]), 0)));
  }
